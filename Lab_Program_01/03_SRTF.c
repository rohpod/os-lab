...
int main() {
  ...
  int pid[n], at[n], bt[n], rem[n], ct[n], tat[n], wt[n], rt[n], done[n],
      started[n];
  ...
  for (int i = 0; i < n; i++) {
    ...
    scanf("%d %d", &at[i], &bt[i]);
    rem[i] = bt[i]; // remaining time = burst time initially
    done[i] = 0;
    started[i] = 0;
  }

  // --- Simulate SRTF (tick by tick) ---
  int time = 0, completed = 0;
  while (completed < n) {

    // Find arrived, not-done process with minimum remaining time
    int idx = -1;
    for (int i = 0; i < n; i++) {
      if (!done[i] && at[i] <= time) {
        if (idx == -1 || rem[i] < rem[idx])
          idx = i;
      }
    }
    ...
    // Capture RT the first time this process gets the CPU
    if (!started[idx]) {
      rt[idx] = time - at[idx]; // RT = first start - AT
      started[idx] = 1;
    }

    rem[idx]--; // run for 1 tick
    time++;
