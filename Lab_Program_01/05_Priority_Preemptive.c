...
int main() {
  ...
  int pid[n], at[n], bt[n], pr[n], rem[n], ct[n], tat[n], wt[n], rt[n], done[n],
      started[n];
  ...
  // --- Simulate Priority (Preemptive, tick by tick) ---
  int time = 0, completed = 0;
  while (completed < n) {

    // Find arrived, not-done process with minimum priority number
    int idx = -1;
    for (int i = 0; i < n; i++) {
      if (!done[i] && at[i] <= time) {
